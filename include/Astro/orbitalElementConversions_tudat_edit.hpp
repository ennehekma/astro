...
Vector6 convertKeplerianToCartesianElements(
    const Vector6& keplerianElements, const Real centralBodyGravitationalParameter, const Real tolerance )
{
  // Check that the Keplerian elements vector contains exactly 6 elemenets and otherwise throw
    ...
    Vector6 convertedCartesianElements_ = keplerianElements;
    ...
    std::vector< Real > positionPerifocal_( 2 );
    // position[ 0 ] = cartesianElements[ 0 ];

    positionPerifocal_[0] = semiLatusRectum_ * cosineOfTrueAnomaly_
            / ( 1.0 + eccentricity_ * cosineOfTrueAnomaly_ );
    positionPerifocal_[1] = semiLatusRectum_ * sineOfTrueAnomaly_
            / ( 1.0 + eccentricity_ * cosineOfTrueAnomaly_ );

    // Definition of velocity in the perifocal coordinate system.
    std::vector< Real> velocityPerifocal_( 2 );

    velocityPerifocal_[ 0 ] = -sqrt( centralBodyGravitationalParameter / semiLatusRectum_ ) * sineOfTrueAnomaly_ ;
    velocityPerifocal_[ 1 ] = sqrt( centralBodyGravitationalParameter / semiLatusRectum_ ) * ( eccentricity_ + cosineOfTrueAnomaly_ ) ;

    // Definition of the transformation matrix.
    Real transformationMatrix_ [2][3]  ;
    ...
    vector<Real> position_ = multiplies(transformationMatrix_ * positionPerifocal_);
